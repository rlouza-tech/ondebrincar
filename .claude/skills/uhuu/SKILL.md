---
name: uhuu
description: Roda o fluxo da Uhuu no pipeline editorial do Onde Brincar (scrape → check-novidades → pipeline-ia → checkpoint → import-sanity). Use quando o Rafa pedir para "puxar novidades da Uhuu", "rodar a Uhuu" ou equivalente, ou quando for a vez da Uhuu na rotina completa (chamada pela skill Orquestradora). Só funciona no Claude Code, rodando no terminal local do projeto — não funciona no Cowork (sandbox não roda pnpm nem git deste repo).
---

# Skill Uhuu — Pipeline editorial (Onde Brincar)

5ª fonte do pipeline editorial, adicionada depois do bloco original de 4 skills decidido na
ADR de US-E4 (`docs/decisions/2026-07-15-us-e4-decomposicao-skills-pipeline-editorial.md`).
Scraper e `check-novidades` já existiam desde a US-S28 (Sprint 14); a US-E13 (Sprint 15)
completou o suporte no `pipeline-ia`/`import-sanity`/schema. Esta skill empacota o fluxo
completo, no mesmo padrão de Clubinho e Sympla (US-E6/E7).
...
```
pnpm import-sanity --source uhuu --execute
```
...
Sem passo de `enrich` separado (diferente do Sympla) — o scraper da Uhuu já sai enriquecido
numa passada só, direto da página de detalhe do evento. Também não precisa de `--headed`
(diferente do Clubinho): o fluxo da Uhuu não passa pelo probe de Cloudflare, roda headless
normalmente.

## Ao terminar

Feche com uma linha de contagem final clara — ex.: "Uhuu: N fichas novas, M criadas como
